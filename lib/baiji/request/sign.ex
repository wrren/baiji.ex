defmodule Baiji.Request.Sign do
  ...
  def string_to_sign(%Sign{canonical_request: req, time: time, request: %Request{operation: op}} = sign) do
    sts = string_to_sign(time, op.region, op.endpoint.endpoint_prefix || op.endpoint.service, req)
  ...
  def signing_key(%Sign{request: %Request{operation: op}, time: time} = sign) do
    %{sign | signing_key: signing_key(op.secret_access_key, time, op.region, op.endpoint.endpoint_prefix || op.endpoint.service)}
  end
  ...
  def add_authorization_header(%Sign{request: %Request{operation: op} = req, time: time} = sign) do
    %{sign | request: %{req | headers: add_authorization_header(
      req.headers,
      op.access_key_id,
      credential_scope(time, op.region, op.endpoint.endpoint_prefix || op.endpoint.service),
      sign.signed_headers,
      sign.signature
    )}}
  end
